...
Run: g++ -std=c++14 -o hw2.exe hw2.cpp && ./hw2.exe && rm hw2.exe
*/
...
int main()
{
    ...
    (Subtract(px, qx)).print();
    
    cout << endl << endl << "p(x)*q(x) = ";
    (Multiply(px, qx)).print();
    
    cout << endl << endl << "p(x)/q(x) = ";
    Rational px_div_qx(px, qx);
    px_div_qx.print();

    cout << endl << endl << "p(x)/q(x)+p(x)*q(x) = ";
    Rational px_rational(px),
             qx_rational(qx);
    (px_div_qx + (px_rational * qx_rational)).print();

    cout << endl << endl << "p(x) + 1 = ";
    (px + Polynomial({1})).print();
    // (px_rational + Rational(Polynomial({1}))).print();
    
    cout << endl << endl << "p(x) + 2 = ";
    (px + Polynomial({2})).print();

    cout << endl << endl << "(p(x)/q(x)) * (1+x^2-3x^4) = ";
    (px_div_qx * (Rational(Polynomial({1,0,1,0,-3})))).print();
    ...
    cout << endl << endl << "p(2) = " << px.evaluateAt(2.0) << endl
    	 << endl
    	 << "p(3)/q(3) = " << px.evaluateAt(3.0)/qx.evaluateAt(3.0) << endl;
